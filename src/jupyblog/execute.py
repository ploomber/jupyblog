...
class ASTExecutor:
    ...
    def __init__(self, wd=None, front_matter=None, img_dir=None, canonical_name=None):
        self._session = None
    ...
    def __call__(self, md_ast):
        ...
        if self.wd:
            ...
            self._session.execute('import os; os.chdir("{}")'.format(str(self.wd)))
...
def parse_info(info):
    if info is not None:
        ...
        return {t.split("=")[0]: t.split("=")[1] for t in elements[1].split(",")}
    else:
        return {}
...
class JupyterSession:
    ...
    def __init__(self, front_matter=None, img_dir=None, canonical_name=None):
        ...
        if self._front_matter.jupyblog.serialize_images:
            serialized = Path(self._img_dir, self._canonical_name, "serialized")
...
def extract_outputs_from_notebook_cell(
